...
const AddEvent = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (data.insertedId) {
        Swal.fire("Success", "You added the event successfully", "success");
  ...
  return (
    <div className="max-w-4xl mx-auto my-8 p-4 bg-gray-100 shadow-lg rounded-lg">
      ...
      <h1 className="text-xl font-bold text-red-500 text-center mb-8">
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 gap-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, events: e.target.value.split(",") })
              }
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm h-10"
              required
            />
        ...
        <button
          type="submit"
          className="mt-4 w-full py-2 px-4 bg-purple-900 hover:bg-purple-500 text-white font-bold rounded-md shadow-md"
        >
          Create Event
        </button>
      </form>
